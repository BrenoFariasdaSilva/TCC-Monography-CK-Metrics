...
Os trabalhados atuais, tais como, ``Students' Perception of Example-Based Learning in Software Modeling Education'' \cite{Tiago.Bonetti-etal:2023}, ``DistFax - A Toolkit for Measuring Interprocess Communications and Quality of Distributed Systems'' \cite{DistFax}, ``Learning from Examples - Instructional Principles from the Worked Examples Research'' \cite{Robert.Atkinson-etal:2000} e ``Using Real Worked Examples to Aid Software Engineering Teaching' \cite{Simone.Tonhao-etal:2021} e ``Uma Plataforma Gamificada de Desafios Baseados Em Worked Examples Extraídos de Projetos de Software Livre Para o Ensino de Engenharia de Software'' \cite{Simone.Tonhao-etal:2022}, até onde se sabe, não foram encontrados trabalhados que estudaram a utilização da evolução de códigos de \gls{sds} por meio de métricas estáticas de código para serem então criados exemplos trabalhados para a \gls{es}.
