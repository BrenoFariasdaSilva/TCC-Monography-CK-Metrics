...
O estado atual da análise nos indica que há vários \textit{commits} onde a tendência das métricas foi decrescente, todavia apenas um pequeno escopo desses \textit{commits} tem mensagens de \textit{commit} relacionadas a melhorias de desempenho, segurança ou troca de algoritmos. Infelizmente, não foi possível correlacionar nenhuma das vulnerabilidades expostas no \textit{``CVE-Details''} \cite{cve-details} com algum \textit{commit} no \textit{``ZooKeeper''}.
