...
Dessa forma, os repositórios escolhidos foram o \cite{KafkaGitHub} e o \cite{ZookeeperGitHub}. Essa escolha baseou-se no fato de ambos os repositórios serem projetos \textit{open-source} disponíveis na plataforma GitHub, onde ambos são mantidos e atualizados de forma ativa para serem utilizados em ambientes do mundo real. Além disso, esses projetos lidam com questões altamente relevantes em \gls{sds}.

O \cite{ZookeeperGitHub} desempenha um papel fundamental na coordenação e gerenciamento de serviços distribuídos. Ele oferece um serviço de consenso altamente confiável para \gls{sds}, garantindo a consistência e a sincronização entre os nós. Essa funcionalidade é crucial para a implementação de serviços distribuídos confiáveis e escaláveis, tornando o \cite{ZookeeperGitHub} uma escolha valiosa para este estudo.

Por outro lado, o \cite{KafkaGitHub} destaca-se no processamento de fluxos de dados distribuídos em larga escala. Ele fornece uma plataforma robusta para a transmissão eficiente de eventos entre diferentes componentes de um \gls{sd}. A capacidade do \cite{KafkaGitHub} lidar com volumes massivos de dados em tempo real o torna uma solução amplamente adotada em ambientes que demandam alto desempenho e escalabilidade.
...
Apesar de terem sido escolhidos inicialmente dois repositórios para serem analisados, o \cite{KafkaGitHub} e o \cite{ZookeeperGitHub}, até o momento foi analisado o apenas o repositório do \cite{ZookeeperGitHub}, devido à limitação do tempo. 
O estado atual da análise nos indica que há vários \textit{commits} onde a tendência das métricas foi decrescente, todavia apenas um pequeno escopo desses \textit{commits} tem mensagens de \textit{commit} relacionadas a melhorias de desempenho, segurança ou troca de algoritmos. Infelizmente, não foi possível correlacionar nenhuma das vulnerabilidades expostas no \cite{cve-details} com algum commit no \cite{ZookeeperGitHub}.

% Mencionar o caso de melhorias de performance do \cite{ZookeeperGitHub}, onde o commit altera inúmeros arquivos mas em si é apenas uma atualização da biblioteca, a qual existe desde o java 1.0 que foi lançado em torno de 1996.
