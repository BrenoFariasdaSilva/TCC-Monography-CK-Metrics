...
\begin{itemize}
    \item \textbf{Ser um repositório \cite{JavaProgrammingLanguage}:} Essencial para viabilizar a aplicação das métricas \gls{ck}.
    \item \textbf{Ser de código aberto (\textit{open-source}):} A escolha por repositórios abertos, hospedados em plataformas como o \cite{Github}, visa garantir a transparência e acessibilidade do código-fonte.
...
O estado atual da análise nos indica que há vários \textit{commits} onde a tendência das métricas foi decrescente, todavia apenas um pequeno escopo desses \textit{commits} tem mensagens de \textit{commit} relacionadas a melhorias de performance, segurança ou troca de algoritmos. Infelizmente, não foi possível correlacionar nenhuma das vulnerabilidades expostas no \cite{cve} com algum commit no \cite{ZookeeperGitHub}.
