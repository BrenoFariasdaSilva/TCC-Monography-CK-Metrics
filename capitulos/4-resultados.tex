...
A análise atual revela a presença de vários \textit{commits} em que observamos uma tendência decrescente no valor das métricas como o \gls{cbo}. Entretanto, é crucial notar que apenas uma parcela restrita desses \textit{commits} inclui mensagens relacionadas a melhorias de desempenho, segurança ou substituição de algoritmos. Lamentavelmente, não conseguimos estabelecer uma correlação direta entre as vulnerabilidades identificadas no banco de dados do \textit{``CVE-Details''} \cite{cve-details} e os \textit{commits} no repositório do \textit{``ZooKeeper''}. Essa lacuna na associação entre vulnerabilidades conhecidas e alterações no código ressalta a complexidade de identificar causalidades diretas entre o histórico do código-fonte e as vulnerabilidades de segurança.
